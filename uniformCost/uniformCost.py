...
def UniformCost(d,n,k):
    ...
    while len(o.l[0])<n:
        o = q.get()
        partition = o.l
        for p in o.adyacents():
            newo=Partition(d.inter_cluster(p), p)
            q.put(newo)
    ...
    for dim in dims:
        for point in points:
            for k in clusters:
                ...
                t1=time.time()
                o=UniformCost(d,point,k)
                opttime=time.time()-t1
                print '%d,%d,%d,%.6f,%.6f'%(k,dim, point, opttime,o.intercluster)
